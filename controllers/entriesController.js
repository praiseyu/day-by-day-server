...
async function getEntry(req, res) {
    ...
    try {
        const entryDetails = await knex("entries").where({ user_id: user_id, entry_date: entryDate }).first();
...
async function saveEditEntry(req, res) {
    ...
    try {
        await knex("entries").where({ user_id: user_id, entry_date: entryDate, trip_id: tripId }).update(changes);
